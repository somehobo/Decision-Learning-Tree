...
bool debug = false;
int num_threads;
vector<vector<vector<float>>> folds;
...
int main(int argc, char const *argv[])
{
  ...
  working_sz = comm_sz - 1;

  if (my_rank == 0)
  {
    int min_size;
    int max_depth;
    int n_folds = working_sz;
    int meta[2];
    ...
    cout << "Testing on "<<working_sz<<" folds"<<endl;
    ...
    // get folds for distribution
    folds = c_v_split(dataset, n_folds);
    int fold_count[1] = {(int)(folds.size())};
    MPI_Bcast(&fold_count, 1, MPI_INT, 0, MPI_COMM_WORLD);
    for (int i = 0; i < fold_count[0]; i++) // for each fold
    {
      int row_count[1] = {(int)(folds[i].size())};
      MPI_Bcast(&row_count, 1, MPI_INT, 0, MPI_COMM_WORLD);
      for (int j = 0; j < row_count[0]; j++)
      {
        int column_count[1];
        column_count[0] = (int)folds[i][j].size();
        MPI_Bcast(&column_count, 1, MPI_INT, 0, MPI_COMM_WORLD);
        MPI_Barrier(MPI_COMM_WORLD);
        MPI_Bcast(folds[i][j].data(), column_count[0], MPI_FLOAT, 0, MPI_COMM_WORLD);
        MPI_Barrier(MPI_COMM_WORLD);
        if (debug)
        {
          cout << "finished broadcasting row " << j << "/" << folds[i].size() << " ";
          for (float item : folds[i][j])
          {
            cout << item << ", ";
          }
          cout << endl;
      ...
      if (debug)
      {
        cout << "finished broadcasting fold " << i << "/" << working_sz << endl;
      }
    }

    if (debug)
    {
      cout << "finished broadcasting folds" << endl;
    }

    vector<float> scores;
    for (int i = 1; i <= working_sz; i++)
    {
      float score_buf[1];
      MPI_Recv(&score_buf, 1, MPI_FLOAT, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      scores.push_back(score_buf[0]);
      if (debug)
      {
        cout<<"recieved Process "<<i<<" score"<<endl;
      }
    }

    if (debug)
    {
      cout << "Finished recieving scores" << endl;
    }
  ...
  else
  {
    ...
    int fold_count[1];
    MPI_Bcast(&fold_count, 1, MPI_INT, 0, MPI_COMM_WORLD);
    folds.resize(fold_count[0]);
    for (int w = 0; w < fold_count[0]; w++)
    {
      int row_count[1];
      MPI_Bcast(&row_count, 1, MPI_INT, 0, MPI_COMM_WORLD);
      folds[w].resize(row_count[0]);
      for (int i = 0; i < row_count[0]; i++)
      {
        int column_count[1];
        MPI_Bcast(&column_count, 1, MPI_INT, 0, MPI_COMM_WORLD);
        folds[w][i].resize(column_count[0]);
        MPI_Barrier(MPI_COMM_WORLD);
        MPI_Bcast(folds[w][i].data(), column_count[0], MPI_FLOAT, 0, MPI_COMM_WORLD);
        MPI_Barrier(MPI_COMM_WORLD);
      }
    }
    // for fold in folds
    ...
    // prediction
    for (int w = 0; w < folds[assigned_fold].size(); w++)
    {
      vector<float> row_copy = folds[assigned_fold][w];
    ...
    vector<float> actual;
    for (int n = 0; n < folds[assigned_fold].size(); n++)
    {
      actual.push_back(folds[assigned_fold][n][folds[assigned_fold][n].size() - 1]); // grab last of each row
    }
    float accuracy = accuracy_metric(actual, predicted);
    float accuracy_send[1] = {accuracy};
    MPI_Send(accuracy_send, 1, MPI_FLOAT, 0, 0, MPI_COMM_WORLD);
  }
